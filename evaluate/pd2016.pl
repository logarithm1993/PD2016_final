...
my $nctu_out_file = $prefix."Eval.gr";
...
my $out_placelog = $prefix."Eval.place.log";
my $out_routelog = $prefix."Eval.route.log";
my $out_evallog  = $prefix."Eval.eval.log";
...
sub runNTUplace3{
    ...
    system("./ntuplace3 -aux $eval_aux -util $urate 2>&1 | tee $out_placelog");
}
#---------------------------------------------------------------------------------------------------------------#
sub runNCTUgr{
    ...
    print "Running NCTUgr...\n";
	 ...
	 system("./NCTUgr ICCAD $eval_aux $ntu_out_file $nctu_setting_file $nctu_out_file 2>&1 | tee $out_routelog");
}
#---------------------------------------------------------------------------------------------------------------#
sub evalRoutability{
    ...
    print "Running Routability Evaluation...\n";
	 ...
	 system("./evaluate_routability.pl -p $eval_aux $ntu_out_file $nctu_out_file 2>&1 | tee $out_evallog");
}
#---------------------------------------------------------------------------------------------------------------#
